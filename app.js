...
//   try {
//     // Find users with purpose "donor" in the database
//     var donors = await blood.find({ purpose: "Donor" });
...
app.get('/acc/donors', async (req, res) => {
  try {
    ...
    const donors = await blood.find({ purpose: 'Donor' })
      .populate('userId', 'name age email phoneNo bloodGroup'); // Populate user details from User model
...
//   var result = await courseInfo.findByIdAndUpdate(req.body.id,req.body);
//   res.send("Updated");
// });
